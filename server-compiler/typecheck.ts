...
export function typeCheckProgram(
  ...
  prog.fundefs.forEach((fundef) => {
    ...
    env.funcs.set(fundef.name, [
      fundef.params.map((param) => param.type),
      fundef.ret,
    ]);
  });
  prog.fundefs.forEach((fundef) => {
...
function typeCheckVarInits(
  ...
  inits.forEach((init) => {
    ...
    if (!checkAssign(init.type, initType.a, classes)) {
      throw new TypeError(
        `Expected type \`${init.type}\`; got type \`${initType.a}\``
      );
    }
...
function typeCheckFunDef(fundef: FunDef<null>, env: TypeEnv): FunDef<Type> {
  ...
  const typedInits = typeCheckVarInits(
    fundef.inits,
    localEnv.vars,
    localEnv.classes
  );
  ...
  if (lastStmt.tag !== "return") {
    if (lastStmt.tag === "pass") {
      ...
      throw new TypeError(
        `All paths must have return for function ${fundef.name}`
      );
    }
  }
...
function typeCheckClassDef(
  ...
  const typedFields = typeCheckVarInits(
    classdef.fields,
    classEnv.fields,
    env.classes
  );
  ...
  classdef.methods.forEach((method) => {
    if (method.name === "__init__") {
      if (method.params.length !== 1 || method.ret !== "None")
        throw new TypeError(
          "Method overriden with different type signature: __init__"
        );
      if (
        typeof method.params[0].type !== "object" ||
        method.params[0].type.class !== classdef.name
      ) {
        throw new TypeError(
          "First parameter of the following method must be of the enclosing class: __init__"
        );
      }
    }
    ...
    switch (stmt.tag) {
      case "assign":
        if (typeof stmt.lvalue === "string") {
          ...
          if (!checkAssign(expectedType, typedValue.a, env.classes)) {
            throw new TypeError(
              `Expected type \`${expectedType}\`; got type \`${typedValue.a}\``
            );
          }
          ...
          if (!checkAssign(expectedType.a, typedValue.a, env.classes)) {
            throw new TypeError(
              `Expected type \`${expectedType.a}\`; got type \`${typedValue.a}\``
            );
          }
          typedStmts.push({
            ...stmt,
      ...
      case "return":
        ...
        if (!checkAssign(env.retType, typedRet.a, env.classes)) {
          throw new TypeError(
            `Expected type \`${env.retType}\`; got type \`${typedRet.a}\``
          );
        }
      ...
      case "if":
        ...
        if (typedCond.a !== "bool")
          throw new TypeError(
            `Condition expression cannot be of type \`${typedCond.a}\``
          );
      ...
      case "while":
        ...
        if (typedWhileCond.a !== "bool")
          throw new TypeError(
            `Condition expression cannot be of type \`${typedWhileCond.a}\``
          );
  ...
  switch (expr.tag) {
    ...
    case "getattr":
      ...
      if (!classData.fields.has(expr.name)) {
        throw new ReferenceError(
          `\`${expr.name}\` is not a member of class \`${obj.a.class}\``
        );
      }
    ...
    case "builtin2":
      ...
      if (arg1.a !== "int" || arg2.a !== "int") {
        throw new TypeError(
          `Cannot apply builtin2 \`${expr.name}\` on types \`${arg1.a}\` and \`${arg2.a}\``
        );
      }
    ...
    case "uniexpr":
      ...
      switch (expr.op) {
        case UniOp.Not:
          if (right.a !== "bool")
            throw new TypeError(
              `Cannot apply operator \`${expr.op}\` on type \`${right.a}\``
            );
          break;
        case UniOp.Neg:
          if (right.a !== "int")
      ...
      switch (expr.op) {
        ...
        case BinOp.Mod:
          if (left.a !== "int" || right.a !== "int") {
            throw new TypeError(
              `Cannot apply operator \`${expr.op}\` on types \`${left.a}\` and \`${right.a}\``
            );
          }
        ...
        case BinOp.Is:
          if (
            (left.a !== "None" && typeof left.a !== "object") ||
    ...
    case "call":
      if (env.funcs.has(expr.name)) {
        ...
        if (actualArgs.length !== typedArgs.length)
          throw new Error(
            `Expected ${actualArgs.length} arguments for ${expr.name} got ${typedArgs.length}`
          );
        for (let index = 0; index < typedArgs.length; index++) {
          if (!checkAssign(actualArgs[index], typedArgs[index].a, env.classes))
            if (typedArgs[index].a !== actualArgs[index])
              throw new TypeError(
                `Expected type \`${actualArgs[index]}\`; got type \`${typedArgs[index].a}\` for parameter ${index}`
              );
        }
        ...
        if (expr.args.length !== 0) {
          throw new TypeError(
            `Expected 0 arguments for ${expr.name} got ${expr.args.length}`
          );
        }
        ...
        if (!methods.has(expr.name)) {
          throw new ReferenceError(
            `There is no method \`${expr.name}\` in class \`${obj.a.class}\``
          );
        }
        const [actualArgs, retType] = methods.get(expr.name);
        if (actualArgs.length - 1 !== typedArgs.length)
          throw new Error(
            `Expected ${actualArgs.length - 1} arguments for ${expr.name} got ${
              typedArgs.length
            }`
          );
        for (let index = 0; index < typedArgs.length; index++) {
          if (
            !checkAssign(actualArgs[index + 1], typedArgs[index].a, env.classes)
          )
            throw new TypeError(
              `Expected type \`${actualArgs[index + 1]}\`; got type \`${
                typedArgs[index].a
              }\` for parameter ${index}`
            );
        }
...
function typeCheckLiteral(literal: Literal<null>): Literal<Type> {
  switch (literal.tag) {
    case "num":
